...
searchstring = "http://libgen.io/search.php?res=100&req=" + title.replace(/[.,\/#!$%\^&\*;:{}=\-_`~()]/g," ").replace("/\s+/g"," ").split(' ').join('+') + '+' + author.split(' ').join('+');
dlstring = "http://booksdl.org/get.php?"
chrome.runtime.sendMessage(
    ...
    function (response) {
		...
		if (response!="unavailable")
		{
			...
			if(response!="customunavailable")
			{
			dlurl = dlstring.concat(response[0][0]);
			btntext = response[1][0];
			alert(btntext);
			$(`<button style="color: white; padding: 10px;border-radius: 4px;text-shadow: 0 1px 1px rgba(0, 0, 0, 0.2);background: rgb(28, 184, 65);"><a href=${dlurl} style="color: white;" target="_blank">$(btntext)</a></button>`).insertAfter(".wtrButtonContainer");
			}
	}
		else
